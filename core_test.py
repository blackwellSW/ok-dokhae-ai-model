...
def main():
    ...
    # 텍스트 정의
    text = """현재 10여개 고객사 공장, 물류센터 등에서 개념검증(PoC)를 진행 중이다. 조선 분야에서는 선박 각 부품이 잘 조립됐는지를 검사하는 휴머노이드 로봇, 물류 센터에서는 박스를 적재하거나 빈 박스를 회수하는 등의 다양한 업무를 수행할 수 있는 휴머노이드 로봇 등의 PoC를 추진하고 있다.특히 LG CNS는 로봇이 수행하기 어려웠던 고부가가치 작업을 자동화하는 영역에 집중하고 있다. 순간적인 판단이 필요하거나 작업 내용이 수시로 바뀌는 공정, 안전상 위험 요소가 있어 사람에게 부담이 컸던 작업 등에 휴머노이드 로봇을 투입함으로써 생산성과 안전성을 동시에 끌어올리는 것이 목표다."""
