#!/usr/bin/env python3
"""
튜닝된 Gemini 모델 성능 테스트 스크립트
Vertex AI Generative AI SDK를 사용한 추론 성능 및 품질 평가
"""
...
def test_tuned_model_inference(
    ...
    """
    튜닝된 Gemini 모델로 추론 테스트

    Args:
        model_name: 튜닝된 모델 이름 (예: ok-dokhae-logic-tutor-v1)
    ...
    try:
        ...
        print(f"❌ 모델 로드 실패: {e}")
        print("\n💡 대안: 직접 모델 ID를 사용해보세요 (예: 9042655206201884672)")
        return []
...
def main():
    parser = argparse.ArgumentParser(description="튜닝된 Gemini 모델 성능 테스트")

    parser.add_argument(
        "--model-name",
        type=str,
        default="9042655206201884672",  # 모델 ID 직접 사용
        help="튜닝된 모델 이름 또는 ID"
    )
    parser.add_argument(
        "--project-id",
        type=str,
    ...
    # 성능 테스트 실행
    results = test_tuned_model_inference(
